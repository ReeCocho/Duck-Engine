...
namespace dk
{
	...
	ShaderPipeline create_shader_pipeline(const vk::Device& logical_device, const ShaderPipelineCreateInfo& info)
	{
		...
		pipeline_layout_info.pushConstantRangeCount = static_cast<uint32_t>(info.push_constant_ranges.size());
		pipeline_layout_info.pPushConstantRanges = info.push_constant_ranges.data();
		...
		pipeline_info.pColorBlendState = &info.color_blending;
		pipeline_info.pDynamicState = &info.dynamic_state;
